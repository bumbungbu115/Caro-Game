...
namespace Tạo_Bàn_Cờ
{
    public partial class Play : Form
    {
        ...
        void NewGame()
        {
            ...
            prcbCoolDown.Value = 0;
            timerCoolDown.Stop();
            ChessBoard.Currentplayer = CuaSoChinh.k;
            ChessBoard.DrawChessBoard(panel1, textBox2, pictureBox1);
            panel1.Enabled = true;
        ...
        private void pictureBox3_Click(object sender, EventArgs e)
        {
            ...
            if (Q == DialogResult.OK)
            {
                ...
                { }
                 ...
                 th.Start();
                this.Hide();
                var cuaSoChinh= new CuaSoChinh(user);
        ...
        {   
            socket.Send(new SocketData((int)SocketCommand.SEND_MESS,Login.title+" : "+ txtmess.Text, new Point()));
        ...
        {
            quitcheck = false;
            string query = "update INFO set TONG+=1,WIN +=1,SCORE+=12 where TENTK = '"+Login.title+"' ";
        ...
        {
            quitcheck = false;
            string query = "update INFO set TONG+=1,LOSE +=1,SCORE-=12 where TENTK = '" + Login.title + "' ";
            using (SqlConnection conn = new SqlConnection(Connection.SeverConnection.stringConnection))
            {
                conn.Open();
                SqlCommand cmd2 = new SqlCommand(query, conn);
